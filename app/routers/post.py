...
def update_post(id: int, post: schemas.PostCreate, db: Session = Depends(get_db),
    ...
    if post_query == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, 
                            detail=f"post with id: {id} doesn't existed")
...
def delete_post(id: int, db: Session = Depends(get_db),
    ...
    post_query = del_post.first()
    if  post_query== None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"post with id: {id} doesn't existed")
    if post_query.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, 
                            detail = "Not authorized to delete")
